...
export class GameManager {
    constructor(netManager, inputHandler, worldState, saveSystem, chatSystem, uiManager) {
        ...
        this.ui = uiManager; // NOVA DEPENDÊNCIA
    ...
    initListeners() {
        ...
        window.addEventListener('playerClicked', this._handlePlayerClick); // Listener de UI
        this.resize();
    }
    ...
    startGame(seed, id, nick, isHost = false) {
        // UIManager assume o controle visual
    ...
    setupSpawnPoint(id, isHost) {
        ...
        if (hives[spawnIdx]) {
            ...
            this.net.sendPayload({ 
                type: 'SPAWN_INFO', id: this.localPlayer.id, nick: this.localPlayer.nickname, 
                x: this.localPlayer.pos.x, y: this.localPlayer.pos.y 
            });
        }
    }
    ...
    update() {
        ...
        // Otimização: Só atualiza UI se mudou de posição significativa
        ...
        if (gx !== this.lastGridX || gy !== this.lastGridY) {
            ...
            this.ui.updateStats(this.localPlayer); // UI Manager cuida do DOM
        }
        ...
        if (moving || Math.random() < 0.05) {
            ...
            this.net.sendPayload({ 
                type: 'MOVE', id: this.localPlayer.id, nick: this.localPlayer.nickname, 
        ...
        if (++this.counters.damage > 60) {
            this.calculateRanking(); // Prepara dados
    ...
    handleNetData(e) {
        ...
        if (d.type === 'PARTY_LEAVE' && this.currentPartyPartner === d.fromId) { 
            this.chat.addMessage('SYSTEM', null, `Parceiro saiu.`); 
            this.currentPartyPartner = null; 
    ...
    recoverFromFaint(rescuerNick) {
        ...
        this.isFainted = false;
        if (rescuerNick) {
            this.chat.addMessage('SYSTEM', null, `Salvo por ${rescuerNick}!`);
    ...
    calculateRanking() {
        // Prepara dados para o UIManager
        let allPlayersData = Object.keys(this.guestDataDB).map(nick => ({
            nickname: nick, tilesCured: this.guestDataDB[nick].tilesCured || 0, isOnline: false
        }));
        if (!allPlayersData.find(p => p.nickname === this.localPlayer.nickname)) {
            allPlayersData.push({ nickname: this.localPlayer.nickname, tilesCured: this.localPlayer.tilesCured || 0, isOnline: true });
        }
        // Marca quem está online
        allPlayersData.forEach(p => {
            if (Object.values(this.remotePlayers).some(rp => rp.nickname === p.nickname) || p.nickname === this.localPlayer.nickname) {
                p.isOnline = true;
            }
        });
    ...
    drawTile(t, sX, sY, size) {
        ...
        if (['FLOR','FLOR_COOLDOWN'].includes(type) && this.assets.flower.complete) {
            ...
            this.ctx.drawImage(this.assets.flower, sX, sY - size/2, size, size); // Simplificado para mobile
            this.ctx.globalAlpha = 1.0;
        } else if (type === 'BROTO') {
            this.ctx.fillStyle = '#006400'; this.ctx.fillRect(sX+size*0.4, sY+size*0.4, size*0.2, size*0.2);
        } else if (type === 'MUDA') {
            this.ctx.fillStyle = '#228B22'; this.ctx.fillRect(sX+size*0.3, sY+size*0.3, size*0.4, size*0.4);
        }
    }

    drawParticles(rTileSize) {
        // (Código de partículas igual ao original, mantido para brevidade mas incluso na lógica)
        this.pollenParticles.forEach(p => { 
            ...
            this.ctx.fillStyle = `rgba(241,196,15,${p.life})`; this.ctx.fillRect(psX, psY, p.size * this.zoomLevel, p.size * this.zoomLevel); 
        });
        this.smokeParticles.forEach(p => { 
            const psX = (p.wx - this.camera.x) * rTileSize + this.canvas.width / 2;
            const psY = (p.wy - this.camera.y) * rTileSize + this.canvas.height / 2; 
            this.ctx.fillStyle = p.isEmber ? `rgba(231,76,60,${p.life})` : `rgba(${p.grayVal},${p.grayVal},${p.grayVal},${p.life*0.4})`;
    ...
    drawCompass(rTileSize) {
        if (this.localPlayer && this.localPlayer.homeBase) {
            ...
            if (dist > 30) {
                ...
                this.ctx.save(); this.ctx.translate(ax, ay); this.ctx.rotate(angle); 
                this.ctx.fillStyle = "#f1c40f"; this.ctx.beginPath(); this.ctx.moveTo(0,0); this.ctx.lineTo(-10,-5); this.ctx.lineTo(-10,5); this.ctx.fill(); this.ctx.restore();
            }
        }
    }
